    ...
    **Warning**: The :attr:`chain` member of this object has the shape:
    ...
    def _propose_stretch(self, p0, p1, lnprob0):
		...
		* ``blob`` - The new meta data blobs or ``None`` if nothing was
		...
		# Renormalise sum of parameters to unity

		q=q-np.log(np.sum(np.exp(q),1)).repeat(q.shape[1]).reshape(q.shape)
		#print(np.sum(np.exp(q)))
		# Don't allow parameters get too negative
		q=np.where(q<-100.,-100.,q)
		assert [(0.99<np.sum(np.exp(q[i,:]))<1.01) for i in range(q.shape[0])]
