...
if len(sys.argv)<3:
	...
	print 'Example: python run_RR_fitter.py /input/input_file.txt /input/setup_file.txt'
	sys.exit(0)
...
print '------------------------------------------------------------------------'
print 'Running RR_fitter on', num_gals, 'galaxies.'
print '------------------------------------------------------------------------'

subprocess.check_call(['python',path_code+'bayesian_force.py',input_file, setup_file])
plotting=str(re.search(r"\'\s*(.*?)\s*\'",linecache.getline(input_file,9)).group(1))
if plotting in ['True']:
	print '------------------------------------------------------------------------'
	print 'Plotting best fit spectra for all galaxies'
	subprocess.check_call(['python',path_code+'plot_spectrum.py',input_file, setup_file])
	print 'Plotting MCMC results for all galaxies'
	subprocess.check_call(['python',path_code+'plot_corner_walkers.py',input_file, setup_file])
